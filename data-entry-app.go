...
func usersHandler(w http.ResponseWriter, r *http.Request) {
  ...
  if err != nil {
    fmt.Fprint(w,"Failed to connect to MySQL database, check ENVIRONMENT variables!\n")
    log.Fatal(err)
  }
  //defer dbc.Close()
  ...
  for rows.Next() {
    ...
    // load user into struct
    this_user.UserId = user_id
    this_user.SystemId = system_id
    users = append(users, this_user)
  }

  // Display to web output
  tmpl.ExecuteTemplate(w, "Show", users)
